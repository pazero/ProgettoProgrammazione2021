...
Motore::Motore(int rect_lines, int rect_cols) {
    infinita = BigMap();    //inizializzo infinita con il costruttore per inzializzare a dus volta i color pairs
    ...
    if(start_menu()) {
        ...
        nodi = 0;   //numero di nodi esplorati
...
void Motore::go_game(){
    while(!dead) {
        //questa funzione prende come argomento un intero che rappresenta il tempo in ms
        //che passa tra un ciclo e l'altro, senza che lo user prema tasti. Più è grande il numero passato,
        //si rallenta il tempo del gioco
        ...
        update_time();  //controllo i bonus attivi
...
//gestisce i controlli dell'eroe
...
bool Motore::start_menu(){
    ...
    p_list aux = head;
    int choice = 1; //numero voce del menù
